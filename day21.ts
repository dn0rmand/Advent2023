...
class Solver {
  ...
  add(x: number, y: number, remainingSteps: number) {
  ...
  solve(): number {
    ...
    for (let step = 1; step <= this.steps; step++) {
      ...
      for (const { x, y } of this.states.values()) {
        this.add(x, y - 1, this.steps - step);
        this.add(x, y + 1, this.steps - step);
        this.add(x - 1, y, this.steps - step);
        this.add(x + 1, y, this.steps - step);
      }
...
export class Day21 extends Day {
  ...
  part1(input: GardenMap): number {
    const solver = new Solver(input, 64);

    solver.solve();

    return solver.blocks[2][2];
  }
